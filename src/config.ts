...
/**
 * Create a loader that fetches JSON content from the CMS.
 * @param name - The CMS resource name.
 * @returns A function returning parsed JSON data.
 */
function makeCMSLoader(name: string): () => Promise<any> {
  return async () => {
    const resp = await fetch(`${cmsPrefix}/${name}.json`);
    return await resp.json();
  };
}

/**
 * Create a loader for mutation annotation data.
 * @param name - The annotation resource name.
 * @returns A function returning data with comments.
 */
...
const genomeViewerConfig = {
  ...
  makePresetLoader: (name: string) => makeCMSLoader(`mutation-viewer/${name}`)
};
