import {consecutiveGroupsBy} from './array-groups';

export interface Mutation {
  AAs: string;
  text: string;
  reference: string;
  position: number;
  isUnsequenced: boolean;
  [key: string]: any;
}

export interface ShortenedMutation extends Mutation {
  posStart: number;
  posEnd: number;
}

/**
 * Merge consecutive deletion mutations into condensed representations.
 *
 * @param mutations - Array of mutation objects.
 * @returns A new array with consecutive deletions merged.
 */
export default function shortenMutationList(
  mutations: Mutation[]
): ShortenedMutation[] {
  const merged: ShortenedMutation[] = [];
  const groups = consecutiveGroupsBy(
    mutations,
    (left, right) => {
      if (process.env.NODE_ENV !== 'production') {
        for (const field of ['AAs', 'text', 'reference', 'position', 'isUnsequenced']) {
          if (!(field in left) || !(field in right)) {
            throw new Error(
              `Unabled to determine if ${JSON.stringify(left)} and ${JSON.stringify(right)} should be merged, field "${field}" is not provided`
            );
          }
        }
      }
      return (
        !left.isUnsequenced &&
        !right.isUnsequenced &&
        left.AAs === right.AAs &&
        left.AAs === '-' &&
        left.position === right.position - 1
      );
    }
  );
  for (const group of groups) {
    if (group.length === 1) {
      const [{text, AAs, position, ...mut}] = group;
      merged.push({
        ...mut,
        position,
        AAs,
        posStart: position,
        posEnd: position,
        text:
          AAs === '-'
            ? `Δ${position}`
            : text.replace('-', 'Δ').replace('Insertion', 'ins')
      });
    } else {
      const leftest = group[0];
      const rightest = group[group.length - 1];
      const {position: posStart, ...mut} = leftest;
      const {position: posEnd} = rightest;
      const reference = group.map(({reference}) => reference).join('');
      merged.push({
        ...mut,
        text: `Δ${posStart}-${posEnd}`,
        reference,
        position: posStart,
        posStart,
        posEnd
      });
    }
  }
  return merged;
}

