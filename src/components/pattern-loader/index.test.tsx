...
vi.mock('../../utils/config-context', () => ({default: {use: () => [{defaultGene: 'S', geneSynonyms: {}, geneReferences: {}, messages: {}}, false]}}));
vi.mock('../../utils/mutation', () => ({sanitizeMutations: (muts: string[]) => [muts, []]}));
vi.mock('uuid', () => ({v5: () => 'uuid'}));
vi.mock('found', () => ({useRouter: () => ({match: {location: {query: {}, state: {patterns: [{uuid: '1', name: 'p1', mutations: ['A']}]}}}})}));

import PatternLoader from './index';
