...
/** Props for {@link NGSResults}. */
export interface NGSResultsProps {
  taskKey?: string;
  progressLookup: Record<string, ProgressEntry>;
  className?: string;
  onAnalyze?: (codfreqs: any[]) => void;
}

/**
 * Display progress and resulting CodFreq files for a FASTQ2CodFreq job.
 */
export default function NGSResults({
  ...
  const {
    ...
    loadedFiles,
    isDownloading
  } = useDownload({name: 'fastq2codfreq_results', multiple: true});
      ...
      >
        {allProgress.map(({step, description, count, total}) => (
          <li
           key={step}
           data-step="step"
           style={{'--percent': count / total} as any}
          >
            <div
             className={classNames(
               style['result-desc'],
