...
interface FASTQItemProps {
  ...
  index: number;
  className?: string;
  onDragStart(file: FASTQFile, e: React.DragEvent): void;
  onDrag(e: React.DragEvent): void;
  onDragEnd(e: React.DragEvent): void;
  onRemove(payload: {index: number; fileName: string}): void;
  draggable?: boolean;
}
...
function FASTQItem({
  ...
  onRemove,
  draggable
}: FASTQItemProps): JSX.Element {
  const handleDragStart = React.useCallback(
    (event: React.DragEvent<HTMLLIElement>) => {
      event.dataTransfer.setData('text', JSON.stringify({fileName: file.name, index}));
      onDragStart(file, event);
      event.dataTransfer.effectAllowed = 'move';
      (event.currentTarget.dataset as any).dragging = '';
    },
  ...
  const handleDragEnd = React.useCallback(
    (event: React.DragEvent<HTMLLIElement>) => {
      onDragEnd(event);
      delete (event.currentTarget.dataset as any).dragging;
    },
    [onDragEnd]
  );
  ...
  return (
    ...
    >
      <FaRegFileAlt
       className={classNames(style['file-icon'], className ? `${className}__file-icon` : null)}
      />
      <span
       className={classNames(style['file-name'], className ? `${className}__file-name` : null)}
      >
        {file.name}
      </span>
      {draggable ? (
        <FaArrowsAlt
         ...
         className={classNames(style.move, className ? `${className}__file-move` : null)}
        />
      ) : null}
      <FaTimesCircle
       ...
       className={classNames(style.remove, className ? `${className}__file-remove` : null)}
      />
    </li>
  );
}

interface FASTQPairItemProps {
  name: string;
  pair: Array<FASTQFile | null>;
  n: number;
  index: number;
  className?: string;
  onDragStart(file: FASTQFile, e: React.DragEvent): void;
  onDrag(e: React.DragEvent): void;
  onDragEnd(e: React.DragEvent): void;
  curDragFile: FASTQFile;
  onSplit(index: number): void;
  onMove(payload: {src: {index: number; fileName: string}; target: {index: number}}): void;
...
export default function FASTQPairItem({
  ...
  onRemove,
  draggable
}: FASTQPairItemProps): JSX.Element {
  const handleSplit = React.useCallback(() => onSplit(index), [onSplit, index]);
    ...
    <>
      ...
      >
        ...
        <input
         ...
         className={classNames(style['edit-pair-name'], className ? `${className}__pair-name` : null)}
         value={name}
        />
        {n === 2 ? (
          <AiOutlineSplitCells
